...
compile(char *file) {
    ...
    if (file == NULL || filename_typeof(file) == 'c') {
        ...
        nxtlab = 0;
        litlab = getlabel();
	for (i=0; i<NUMBER_OF_LIBEXT; i++) libext[i]=0;
        defmac("end\tmemory");
...
//    output_string("\n;\tFront End for ASXXXX (2.8,13/01/20)");
	output_string("\n;\tFront End for macros80.inc (11-24-91)");
}
...
dumpglbs() {
    ...
    list_size = 0;
    for(i=0; i<NUMBER_OF_LIBEXT; i++) {
	if (!libext[i]) continue;
	if (list_size % 5 == 0) {
		if (list_size) newline();
		output_with_tab("extrn\t");
	} else output_byte(',');
	output_string("@c");
	output_string(libname[i]);
	list_size++;
    }
    if (list_size) newline();
}
